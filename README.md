...
- `.env.local` for client/Vite vars (`VITE_FIREBASE_*`, optional **`VITE_SENTRY_DSN`**, optional `VITE_APP_VERSION`, optional `VITE_APP_ORIGIN`, `VITE_DEPLOYMENTS_URL`)
...
- Optional client variables:
  ...
  - `VITE_DEPLOYMENTS_URL` — Admin “Deployments” card (default in code: `https://vercel.com/maulbogats-projects/watchlist/deployments`; set to override)
...
4. **Movie lists** are stored under **`users/{uid}/personalLists/{listId}`** (plus optional **shared lists** in **`sharedLists/{listId}`**). **Status** (to-watch / watched / maybe-later / archive) is stored **per list** on each doc’s **`watched`**, **`maybeLater`**, and **`archive`** arrays — not as a single global field on **`titleRegistry`**. The **`users/{uid}`** document holds profile fields (e.g. country, **`defaultPersonalListId`**) and optional **`upcomingDismissals`** — not the row arrays. Canonical title metadata lives in **`titleRegistry/{registryId}`** (client read-only; writes via **`api/*`** / scripts). Users add titles via the bookmarklet; no legacy **`catalog`** collection is used.
...
The project was migrated from **Netlify** to **Vercel**: former **`netlify/functions/`** handlers live under root **`api/*.js`**. **`src/api-lib/vercel-adapter.js`** keeps Netlify-shaped handler wiring while exposing standard Node **`(req, res)`** to Vercel. **`vercel.json`** replaces Netlify scheduled functions with **Vercel Cron** (e.g. **`/api/check-upcoming`** at **03:00 UTC**). On the **Hobby** plan, **12** serverless function slots are available by default; this repo defines **9** serverless API routes (including **`api/admin/[segment].js`**, which serves all **`/api/admin/*`** admin endpoints) — confirm your Vercel plan’s limits. **`vercel.json`** sets **`npm run build:react`**, **`outputDirectory`: `dist/`** (includes `index.html`, `add.html`, hashed assets), SPA rewrite (excluding **`/api/*`**), cron, and per-function **`maxDuration`** (**60s** for heavy routes including **`/api/whatsapp-webhook`** and **`/api/admin/[segment]`**; **30s** for **`/api/whatsapp-verify`**).

**Admin “Deployments” card:** set **`VERCEL_API_TOKEN`** and **`VERCEL_PROJECT_ID`** in Vercel env so **`/api/admin/external-status?service=vercel`** can read the latest deployment.

**Admin “GCS Backup” card:** **`/api/admin/external-status?service=gcs`** uses **`@google-cloud/storage`** with **`FIREBASE_SERVICE_ACCOUNT`** to list the **`movie-trailer-site-backups`** bucket and surface the newest export folder (**SUCCESS** if the export is within **48 hours**, **WARNING** if older). Grant the Firebase service account **`storage.objects.list`** on that bucket (for example **Storage Object Viewer**).
...
**Shared list: put every title on the “To Watch” tab** (keeps `items`; clears `watched` / `maybeLater`; removes legacy `archive` field):
...
**Remove legacy `archive` field** from all `sharedLists` and `personalLists` documents (titles that were only archived stay on the list as **To Watch**):
...
- **Titles on more than one list** (same `registryId` in `items` on multiple `sharedLists` / `personalLists` docs, or legacy `users.{uid}.items`): `node scripts/list-titles-on-multiple-lists.mjs`
- **My list + Our list (local report only):** union of titles on your default personal list and the shared list named like “Our list” — set **`WATCHLIST_MY_LIST_UID`** in **`.env`**, then `node -r dotenv/config scripts/list-my-list-and-our-list-titles.mjs` (prints and writes **`backups/my-list-and-our-list-titles.txt`**)
- **My list → Archive (bulk):** move everything on the **To Watch** tab (not Watched, not already Archive) to **Archive** — `node -r dotenv/config scripts/move-my-list-to-watch-to-archive.mjs` (dry run) then `… --write` with **`WATCHLIST_MY_LIST_UID`** + Admin credentials. Targets the personal list named **“My list”** (same as the **MY LIST** selector), not necessarily **`defaultPersonalListId`**. Optional **`WATCHLIST_PERSONAL_LIST_ID`** forces a specific `personalLists` doc id.
- **Consolidate → My list + To Watch:** merge every title from **all** of that user’s **`personalLists`** into the list resolved as **“My list”** (empty **`watched`** / **`maybeLater`** / **`archive`**); optional **`--include-shared`** also pulls titles from **shared lists** where the user is a member and clears those shared docs. Writes **`backups/pre-consolidate-my-list-*.json`** first, then sets **`users/{uid}.defaultPersonalListId`** to that **My list** doc when it differed. `node -r dotenv/config scripts/consolidate-all-titles-to-my-list-to-watch.mjs` (dry run) then `… --write` [ `… --include-shared` ] with **`WATCHLIST_MY_LIST_UID`** + Admin credentials.
- **Our list — restore Keshet’s 42 titles:** add or update a fixed set of **`registryId`**s on the shared list named like **“Our list”** with **`addedBy*`** set to Keshet’s Firebase Auth user; optional **`--removeFromMyList`** (requires **`WATCHLIST_MY_LIST_UID`**) removes those ids from the resolved **My list** personal doc. `node -r dotenv/config scripts/ensure-keshet-titles-on-our-list.mjs --dry-run` then `… --write` [ `… --removeFromMyList` ].
- **Clear all favorites:** deletes the **`favorites`** map on **`users/{uid}`** (per-account star/heart). Writes **`backups/pre-clear-favorites-*.json`** before **`--write`**. `node -r dotenv/config scripts/clear-all-user-favorites.mjs` then `… --write --all-users` (every user who has favorite keys) or `… --write --uid <uid>` / **`WATCHLIST_MY_LIST_UID`** for one account.
...
- **Recommendations:** below "Up next", a horizontal strip of **"Recommended for you"** (personal lists) / **"Recommended for this list"** (shared lists) cards driven by pre-computed `recommendations/{listId}` Firestore docs. Each card shows poster, title/year, type badge (MOVIE/SERIES), and an explanation ("Because you watched: …"). Clicking opens the **trailer modal**. **Dismiss (✕)** removes the card immediately and writes `users/{uid}.dismissedRecommendations.{tmdbId}` for future pipeline filtering. Already-dismissed and already-on-list titles are filtered client-side. **Skeleton strip** while loading. TanStack Query with **1-hour** stale time.
...
- **Admin (`/admin`, admin users only):** link from the main admin header to **`/admin/list-matrix`** — paginated **catalog × lists** grid (one radio group per title per list: **To watch** / **Watched** / **Archive** / **Not on list**; **Submit** applies moves and status changes in bulk via existing client Firestore helpers). **header** link **Switch to prod** / **Switch to local** (opens the other admin URL in a new tab; prod target uses **`VITE_APP_ORIGIN`** or default **`https://watchlist.maulbogat.com`**), **ACTIVITY (LAST 24H)** from Axiom (**`/api/admin/external-status?service=axiom`**, dataset **`watchlist-prod`**), **SENTRY — LAST 24H** unresolved issue count (**`?service=sentry`**, needs **`SENTRY_READ_TOKEN`** + **`SENTRY_PROJECT`**), Firestore read **quota usage** bars, **Data Quality** stats (**`titleRegistry`** field-gap counts; **titles not on any list** — one header row (title on the left; **Scan now** always; **Show/Hide** at the far right when count **> 0**); **Scan now** is the same control before and after a result (including **`sessionStorage`** restore) → **`GET /api/admin/catalog-orphans`** on demand; last results stay cached in **`sessionStorage`** for the tab session; per-row **`POST /api/admin/delete-registry-orphan`** updates the UI from the prior scan plus the delete response without re-running the orphan scan; optional **`meta/catalogHealthExclusions`** doc with **`missingTmdbId`** array to omit known no-TMDB titles from missing-`tmdbId` counts/lists; **Refresh** reloads **`titleRegistry`** stats only, not the orphan scan; Data Quality list rows with **0** issues omit **Show/Hide** and use a static header, and **0 titles not on any list** still shows exiting rows until the delete animation finishes), **Upcoming Check Job** card (**`checkUpcomingEnabled`** via **`GET`/`POST /api/admin/job-config`**), **GitHub Backup** card (latest Actions run via **`/api/admin/external-status?service=github`**, optional **`GITHUB_TOKEN`**; **scheduled JSON export** on/off via **`githubBackupEnabled`** on the same job-config API), **GCS** + **Vercel** deployment status cards, and **Service Links** (Firebase, **Vercel env vars**, **Meta WhatsApp** dev console, **Google Cloud** billing + **project dashboard** + **Cloud Storage** backup bucket + **Cloud Scheduler** (Firestore export job), GitHub, TMDB, Trakt, etc.).
- **Status tabs:** **All**, **To Watch** (**includes “maybe later”** rows), **Watched** — list membership and **`watched`** / **`maybeLater`** arrays in Firestore. **Recently Added** is not a separate tab; use **sort** options **Date Added (New → Old)** and **Date Added (Old → New)**.
