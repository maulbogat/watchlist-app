...
| `src/pages/AdminPage.tsx` | Admin-only dashboard: **Data Quality** (**`titleRegistry`** counts + expandable sample rows; optional **`meta/catalogHealthExclusions`** filters missing-`tmdbId` stats; **`POST /api/catalog-health`** per-title thumb fix), Firestore read-quota bars (**`meta/usageStats`** via **`getFirestoreUsageStats`**), upcoming job controls, GitHub backup + Vercel deployment status, **Service Links** (Vercel env vars, Meta WhatsApp console, Google Cloud billing + project dashboard + **GCS** **`movie-trailer-site-backups`** + **Cloud Scheduler** for **`firestore-daily-export`**, Firebase, etc.). | **`titleRegistry`** (client read + **`getCountFromServer`** / **`getDocs`** with field projection), **`meta/usageStats`**, **`meta/catalogHealthExclusions`** (admin UID), `fetch` to admin APIs | — | `fetch` → `external-status`, `admin-job-config`, **`catalog-health`**; external HTTPS links |
