...
- **Serverless API routes** (root **`api/*.js`**, **`vercel.json`** rewrites + cron **`/api/check-upcoming`**):  
  ...
  - `admin-catalog-orphans.js` — Admin: **`GET`**, Bearer token + admin UID; lists **`titleRegistry`** documents whose id is not referenced as **`registryId`** on any **`sharedLists`** or **`users/*/personalLists`** row (plus legacy **`users.items`**). Response includes **`count`**, **`orphans`** (title/year summaries, capped at **1500**), **`registryDocCount`**, **`referencedDistinctCount`**, **`truncated`** / **`omitted`** when capped. Scan logic: **`src/api-lib/catalog-orphan-scan.cjs`**.  
  - `admin-delete-registry-orphan.js` — Admin: **`POST`** JSON **`{ registryId }`**, Bearer token + admin UID; re-runs referenced-id scan (**`scanReferencedRegistryIds`** in **`catalog-orphan-scan.cjs`**); if the id is still unreferenced, deletes **`titleRegistry/{registryId}`**. **`409`** if the id is on a list; **`404`** if the doc is missing.  
...
flowchart LR
  ...
  subgraph VercelAPI["Vercel serverless (13 routes)"]
    ...
    API_ORPH["admin-catalog-orphans"]
    API_DEL_ORPH["admin-delete-registry-orphan"]
