...
| `scripts/*.js`, `scripts/*.mjs`, `scripts/lib/*` | Maintenance, backup (**`backup-firestore.js`** v4 adds **allowedUsers**, **invites**, **phoneIndex**, **upcomingChecks** to the JSON export alongside existing collections); **`restore-from-backup.js`** restores those four when present (script warns on **allowedUsers** / **phoneIndex**); migration (titleRegistry model); **`seed-allowed-users.mjs`** seeds **`allowedUsers`**; **`catalog-not-on-any-list.mjs`** reports **`titleRegistry`** docs not referenced as **`registryId`** on any **`sharedLists`** or **`users/*/personalLists`** row (plus legacy **`users.items`**). | Admin (typical) | Varies | TMDB, OMDb, etc. |
