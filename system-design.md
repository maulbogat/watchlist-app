...
**Client-side display:** `src/components/RecommendationsSection.tsx` renders a horizontal strip below "Up next" on all non-archive tabs. Uses `useRecommendations` (TanStack Query, 1-hour stale time) and `useDismissedRecommendations` hooks (both in `src/hooks/useRecommendations.ts`). Items already on the current list or previously dismissed are filtered client-side before display. Dismissals are written to `users/{uid}.dismissedRecommendations.{tmdbId}` via `dismissRecommendation()` in `src/firebase.ts` (pipeline will use this map in a future iteration).
...
| `src/pages/AdminListMatrixPage.tsx` | Admin-UID-only (**`src/config/admin.ts`**): paginated **`titleRegistry`** rows (**`orderBy(documentId())`**, **100** per page) × one column group per personal + shared list the signed-in user can access. Four radios per list (**To watch** = active queue incl. **maybe-later**, **Watched**, **Archive**, **Not on list**). **Submit** applies diffs via **`src/data/admin-list-matrix.ts`** (**`applyAdminListMatrixDiffs`**) using **`src/data/titles.ts`** (**`addTitleToList`**, **`removeTitleFromList`**, **`setTitleStatus`**) and **`updateRegistryListStatus`** in **`src/firebase.ts`**; invalidates list queries after success. Pure helpers + tests: **`src/lib/admin-list-matrix.ts`**. | **`titleRegistry`**, **`users/{uid}/personalLists/*`**, **`sharedLists/*`** (membership arrays on list docs) | Same writes as main grid (list **`items`** / **`watched`** / **`maybeLater`** / **`archive`**, **`titleRegistry.listStatus`**) | — |
