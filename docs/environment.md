...
| **Local script: My list + Our list report; My list bulk archive** | `FIREBASE_SERVICE_ACCOUNT` or **`serviceAccountKey.json`**, plus **`WATCHLIST_MY_LIST_UID`** (Firebase Auth **`uid`**). Optional **`WATCHLIST_PERSONAL_LIST_ID`** — Firestore id of the **`personalLists`** doc when you must override resolution (**`scripts/lib/resolve-my-list-ref.mjs`**: explicit id → list named “My list” → **`defaultPersonalListId`**). Used only by **`scripts/list-my-list-and-our-list-titles.mjs`** and **`scripts/move-my-list-to-watch-to-archive.mjs`**; not read by the deployed app |
