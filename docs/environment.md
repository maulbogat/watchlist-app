...
| **Local script: My list + Our list report; My list bulk archive; consolidate to My list + To Watch; clear favorites** | `FIREBASE_SERVICE_ACCOUNT` or **`serviceAccountKey.json`**, plus optional **`WATCHLIST_MY_LIST_UID`** (Firebase Auth **`uid`**) for single-user scripts. Optional **`WATCHLIST_PERSONAL_LIST_ID`** — Firestore id of the **`personalLists`** doc when you must override resolution (**`scripts/lib/resolve-my-list-ref.mjs`**: explicit id → list named “My list” → **`defaultPersonalListId`**). Used by **`scripts/list-my-list-and-our-list-titles.mjs`**, **`scripts/move-my-list-to-watch-to-archive.mjs`**, **`scripts/consolidate-all-titles-to-my-list-to-watch.mjs`**, and optionally **`scripts/clear-all-user-favorites.mjs`** (or pass **`--all-users`** / **`--uid`**); not read by the deployed app |
