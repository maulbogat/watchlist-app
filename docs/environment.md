...
| `VITE_DEPLOYMENTS_URL` | No | Admin “Deployments” card **Open deployments** link; code default **`https://vercel.com/maulbogats-projects/watchlist/deployments`** |
...
| Variable | Purpose |
|----------|---------|
| `FIREBASE_SERVICE_ACCOUNT` | Base64 JSON — Firebase Admin SDK in API routes; also used by **`/api/admin/external-status?service=gcs`** (**`@google-cloud/storage`**) if that account can list **`movie-trailer-site-backups`** |
...
| `TMDB_API_KEY` | `check-upcoming`, `trigger-upcoming-sync`, `add-from-imdb`, **`/api/admin/catalog-health`**, scripts |
| `OMDB_API_KEY` | `add-from-imdb` |
| `AXIOM_TOKEN` | Server-side Axiom ingest (`log-client-event`, function loggers) and **`GET /api/admin/external-status?service=axiom`** (Admin **Activity** APL query on **`watchlist-prod`**) |
...
| `SENTRY_READ_TOKEN` | Optional — **`GET /api/admin/external-status?service=sentry`** (Admin **SENTRY — LAST 24H** card); Sentry API auth with **read** scope; **503** on that route if unset |
...
| **Admin job config** (`checkUpcomingEnabled`, `githubBackupEnabled`) | `FIREBASE_SERVICE_ACCOUNT` — **`/api/admin/job-config`** (no end-user Firebase token check; same-origin Admin UI + Admin SDK) |
| **Admin Axiom activity (24h)** | `AXIOM_TOKEN` — **`/api/admin/external-status?service=axiom`** (dataset **`watchlist-prod`** in APL) |
...
| **Admin GitHub backup status** | `GITHUB_TOKEN` optional for public repo; often required for private — **`/api/admin/external-status?service=github`** |
| **Admin Vercel deployment status** | `VERCEL_API_TOKEN`, `VERCEL_PROJECT_ID` — **`/api/admin/external-status?service=vercel`** |
| **Admin GCS backup status** | `FIREBASE_SERVICE_ACCOUNT` — **`/api/admin/external-status?service=gcs`**; IAM on **`movie-trailer-site-backups`** must allow **`storage.objects.list`** for that service account |
...
| **Admin orphan catalog titles** | `FIREBASE_SERVICE_ACCOUNT` — **`GET /api/admin/catalog-orphans`** and **`POST /api/admin/delete-registry-orphan`** (Firebase ID token + admin UID) |
| **Local script: My list + Our list report; My list bulk archive; consolidate to My list + To Watch; clear favorites** | `FIREBASE_SERVICE_ACCOUNT` or **`serviceAccountKey.json`**, plus optional **`WATCHLIST_MY_LIST_UID`** (Firebase Auth **`uid`**) for single-user scripts. Optional **`WATCHLIST_PERSONAL_LIST_ID`** — Firestore id of the **`personalLists`** doc when you must override resolution (**`scripts/lib/resolve-my-list-ref.mjs`**: explicit id → list named “My list” → **`defaultPersonalListId`**). Used by **`scripts/list-my-list-and-our-list-titles.mjs`**, **`scripts/move-my-list-to-watch-to-archive.mjs`**, **`scripts/consolidate-all-titles-to-my-list-to-watch.mjs`**, **`scripts/ensure-keshet-titles-on-our-list.mjs`** (with **`--removeFromMyList`**), and optionally **`scripts/clear-all-user-favorites.mjs`** (or pass **`--all-users`** / **`--uid`**); not read by the deployed app |
