/**
 * Watchlist — Recommendation Engine v4: Build TMDB Similar Cache (Phase 1b)
 *
 * Fetches TMDB similar titles for every title in titleRegistry and writes
 * two index files to disk:
 *   data/tmdb-similar-forward.json   — catalog title → its TMDB similar titles
 *   data/tmdb-similar-inverted.json  — similar tmdbId → which catalog titles point to it
 *
 * Run:
 *   node scripts/build-similar-cache.mjs                  # full build
 *   node scripts/build-similar-cache.mjs --refresh-days 7 # skip titles updated within 7 days
 *   node scripts/build-similar-cache.mjs --dry-run         # show what would be fetched
 */
...
const FORWARD_INDEX_PATH  = resolve(__dirname, "../data/tmdb-similar-forward.json");
const INVERTED_INDEX_PATH = resolve(__dirname, "../data/tmdb-similar-inverted.json");
...
// ─── TMDB fetch (no cache — similar data changes, cache would go stale) ──────
...
async function main() {
  ...
  // 5. Fetch TMDB similar titles for each title
  let fetched = 0;
  let errors = 0;
  let zeroRecs = 0;
  let totalRecs = 0;
  ...
  for (let i = 0; i < toFetch.length; i++) {
    ...
    const path = `/${mediaType}/${tmdbId}/similar`;
    const data = await fetchTmdb(path);

    const lastUpdated = new Date().toISOString();

    if (!data) {
      console.warn(`  Warning: TMDB returned null for ${imdbId} (${sourceTitle}) — storing empty similar`);
      errors++;
      forwardIndex[imdbId] = {
        tmdbId,
        title: sourceTitle,
  ...
  console.log(`  ${totalRecs} similar titles gathered this run, avg ${avgRecs} per title`);
  if (zeroRecs > 0) console.log(`  ${zeroRecs} titles returned 0 similar from TMDB (possible bad tmdbId or obscure title)`);
  if (errors > 0) console.log(`  ${errors} titles had TMDB errors (stored with empty similar)`);
  console.log();
  ...
  console.log(`${uniqueRecs} unique similar titles\n`);
